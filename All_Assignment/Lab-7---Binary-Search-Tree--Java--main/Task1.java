...
public class Task1 {
    ...
    public static Integer lowestCommonAncestor( BSTNode root, Integer x, Integer y ){
        // TO DO
        int ans=0;
        if((int)root.elem>x && (int)root.elem>y ){
            ans=lowestCommonAncestor(root.left,x,y);
        }
        else if((int)root.elem<x && (int)root.elem<y ){
            ans=lowestCommonAncestor(root.right,x,y);
        }
        else{
